...
const EditarPrestamos = ({prestamo}) => {
  ...
  const obtenerDatosEstudiantes = async () => {
    const url = `https://afternoon-caverns-98117.herokuapp.com/api/estudiantes`;
...
const obtenerDatosLibro = async () => {
  const url = `https://afternoon-caverns-98117.herokuapp.com/api/libros`;
  const response = await axios.get(url);
  console.log('response',response.data);
 ...
 const formik = useFormik({
  ...
  onSubmit: prestamo => {
      try {   
          ...
          let config={
            method:'put',
            url:`https://afternoon-caverns-98117.herokuapp.com/api/prestamos/${prestamo.id}`,
            headers:{
